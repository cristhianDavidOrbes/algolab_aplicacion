...
/// <summary>
/// Instancia y conecta el tema de Abstraccion con el objeto frontal calibrado.
/// </summary>
public class AlgoLabAbstractionThemeController : MonoBehaviour
{
    [Header("Referencias")]
    ...
    public void StartTheme()
    {
        ...
        {
            Debug.LogError("ABSTRACCION: no se encontro ManualPanelSpawnManager.");
            return;
        }

        if (themeVisualPrefab == null)
        {
            Debug.LogError("ABSTRACCION: falta el prefab visual del tema.");
            return;
        }
    ...
    {
        float elapsed = 0f;
        AlgoLabAbstractionThemeVisual foundVisual = null;
        ...
        while (elapsed < maximumConnectWait && myGeneration == connectionGeneration)
        {
            ...
            {
                foundVisual = candidate.GetComponentInChildren<AlgoLabAbstractionThemeVisual>(true);
                if (foundVisual != null)
                {
                    break;
                }
        ...
        if (foundVisual == null)
        {
            Debug.LogError("ABSTRACCION: no se pudo conectar el visual frontal.");
            yield break;
        }
        ...
        if (showDebug)
        {
            Debug.Log("ABSTRACCION: visual conectado; secuencia iniciada.");
        }
    }
    ...
    private void HandleSequenceFinished()
    {
        ...
        {
            Debug.Log(
                "ABSTRACCION: controlador recibio el final del audio 8."
            );
        }
